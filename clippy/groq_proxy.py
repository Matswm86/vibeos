...
USER_AGENT = "Mozilla/5.0 (X11; Linux x86_64) VibeOS-Vibbey/0.4.0"
...
def _read_first_line(path: Path) -> str | None:
    ...
    try:
        ...
        # Support "key=...\n" files too — take the first non-empty line
...
def _normalize_groq_reply(data: dict[str, Any]) -> dict[str, Any]:
    ...
    Groq/OpenAI returns ``{"choices": [{"message": {...}}]}``.

    We normalize so the frontend treats both backends identically.
    """
...
def chat(
    ...
    # Total failure — surface the Ollama error (most informative)
