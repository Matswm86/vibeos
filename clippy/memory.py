"""Vibbey persistent memory — JSON file at ``~/.vibeos/vibbey-memory.json``.

Stores across sessions:
  * ``user`` — profile the user has shared (name, experience_level, tone)
  * ``facts`` — key/value things Vibbey has learned ("user prefers magenta")
...
def load() -> dict[str, Any]:
    ...
    try:
        ...
        # Forward-compat: add missing top-level keys from the empty template
        ...
        # Back up the broken file, start fresh
...
def summarize_for_prompt() -> str:
    ...
    Keeps the model grounded in what Vibbey knows about the user + their
    install, without dumping the entire JSON blob.
    """
    mem = load()
    lines: list[str] = []
