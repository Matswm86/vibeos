...
Design principles:
  ...
  * Read-only by default. No writes, no network requests, no sudo, no config edits.
  * Destructive ops (rm, dd, mkfs, umount, etc.) are NOT in the allowlist.
    If Vibbey ever needs to run one, a KDialog confirmation step will be
    required at that point — for now they simply do not exist in this list.
...
POSTs to ``/api/run`` with ``{tool_id: "gh_auth_status"}`` → backend validates,
...
# Model name regex: allow alphanumerics, dots, hyphens, colons, slashes.
...
def list_tools() -> list[dict[str, str]]:
    ...
    return [
        {"id": key, "description": spec.description, "accepts_arg": spec.accepts_arg}
