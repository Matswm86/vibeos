...
Total size budget: ~8 KB of markdown fits comfortably in qwen2.5:3b's and
llama-3.3-70b's context windows with room to spare for conversation history.
"""
...
# Order matters — identity first, then capabilities, then fault recovery.
...
def load_knowledge_pack() -> str:
    ...
    Loaded fresh on each call so `python3 -m vibbey` picks up doc edits
